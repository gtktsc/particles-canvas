...
import {
  ...
  Particle,
} from "@/features/simulation/model/Particle";
...
describe("Particle", () => {
  ...
  it("integrates acceleration, damping, bounds, then clears acceleration", () => {
    const particle = new Particle(100, 100, 100, "electron");
    particle.position = new Vector3(49, 0, 0);
    particle.velocity = new Vector3(2, 0, 0);

    particle.applyForce(new Vector3(1, 0, 0));
    particle.integrate(1, 0.5, { width: 100, height: 100, depth: 100 });

    expect(particle.position.x).toBe(50);
    expect(particle.velocity.x).toBeCloseTo(-1.5 * BOUNDARY_RESTITUTION);
    expect(particle.acceleration).toMatchObject({ x: 0, y: 0, z: 0 });
  });
